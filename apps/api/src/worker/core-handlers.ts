...
export function createCoreWorkerHandlers(
  ...
  handlers.set("engagement.rollup_day", async ({ dataPlane, job }) => {
    ...
    if (Number.isNaN(configuredDay.getTime())) {
      throw new WorkerJobError(
        "Engagement rollup day is invalid.",
        "invalid_rollup_day",
        true,
      );
    }
  ...
  handlers.set("listings.mark_stale", async ({ dataPlane, job }) => {
    const ageSeconds = Math.max(
      60,
      numericPayload(job.payload, "ageSeconds", 86_400),
    );
    const limit = Math.max(
      1,
      Math.min(5_000, numericPayload(job.payload, "limit", 500)),
    );
    const occurredAt = now();
    const cutoff = new Date(occurredAt.getTime() - ageSeconds * 1_000);
    const records = await dataPlane.listings.markStale(
      cutoff,
      occurredAt,
      limit,
    );
