...
| Final quality evidence      | mixed                | final-code local gates/five repeats pass; production smoke blocked | current CI plus authorized HTTPS smoke                            |
| PostgreSQL/deployment proof | operational evidence | local migration/reliability/restart/restore pass; Docker absent    | current CI, Compose, encrypted off-host restore, HA/PITR          |
