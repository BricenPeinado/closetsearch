...
provider-specific authorization references for the five real adapters. Never
resolve that blocker by enabling mock inventory.
...
Metrics, operations state, and provider state require
`Authorization: Bearer $OPERATIONS_BEARER_TOKEN` whenever that token is
configured and always in production. Do not put the token in a URL, shell
history, log, or monitoring label.
...
- `OPERATIONS_BEARER_TOKEN` of at least 32 secret characters
...
When outbound alerts are enabled, both API and worker also require
`ALERT_DELIVERY_ENABLED`, `ALERT_PUBLIC_BASE_URL`, `AUTH_SESSION_PEPPER`, and
the selected transport configuration. Resend requires
`EMAIL_WEBHOOK_SECRET`; Twilio verifies callbacks with `TWILIO_AUTH_TOKEN`.
...
4. Clean PostgreSQL migration reaches version `007` without drift.
...
2. Build and scan immutable API/worker/web images. CI resolves every image from
   `docker compose config --images` and uses the full-SHA-pinned Grype action to
...
9. Run:
   ...
   CLOSETSEARCH_EXPECTED_PROVIDER_IDS=ebay,grailed,depop,yahoo-auctions-jp,mercari-jp \
   CLOSETSEARCH_OPERATIONS_BEARER_TOKEN="$OPERATIONS_BEARER_TOKEN" \
...
- `/operations/status`: sanitized durable job/checkpoint/provider state; no job
  ...
  messages; production bearer token required
- `/providers/health`: capability/config/authorization-safe provider state; no
  secret values; production bearer token required
...
CI defines real PostgreSQL migration/backup/restore and container jobs, pins
external actions to immutable commit SHAs, and gates every configured Compose
image on high/critical Grype findings. Local PostgreSQL 17.10 evidence covers
migrations, reliability cases,
